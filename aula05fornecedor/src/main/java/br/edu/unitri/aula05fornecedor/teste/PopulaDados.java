...
public class PopulaDados {
	...
	private static void populaFornecedores() {
		...
		produtoDAO.insereProduto(new Produto("havaianas top", fornecedorDAO
				.findById(f1.getCodigo())));
		produtoDAO.insereProduto(new Produto("havaianas color", fornecedorDAO
				.findById(f1.getCodigo())));
		produtoDAO.insereProduto(new Produto("lingui�a toscana", fornecedorDAO
				.findById(f1.getCodigo())));
		produtoDAO.insereProduto(new Produto("havaianas top", fornecedorDAO
				.findById(f1.getCodigo())));
	}
